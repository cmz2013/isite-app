...
import org.isite.bi.data.constants.CacheKeys;
...
public class InviteRankCache {
    ...
    public List<InviteRank> getInviteRank(int activityId) {
        ...
        return redisTemplate.opsForList().range(CacheKeys.OPERATION_INVITE_RANK_PREFIX + activityId, Constants.ZERO, -1);
    }
    ...
    public void updateInviteRank(int activityId) {
        String key = CacheKeys.OPERATION_INVITE_RANK_PREFIX + activityId;
        List<InviteRank> inviteRanks = inviteRankService.findInviteRanks(activityId);
        //删除现有列表
        redisTemplate.delete(key);
        // 批量插入新数据。rightPushAll是用于将多个值一次性推入列表的右侧（末尾）
        redisTemplate.opsForList().rightPushAll(key, inviteRanks);
        // 设置缓存过期时间为3天
        redisTemplate.expire(key, Duration.ofDays(Constants.THREE));
    }
